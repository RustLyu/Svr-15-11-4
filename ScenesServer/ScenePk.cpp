...
/*					if ((int)charbase.sp - 2>=0)
					...
					{
						return ScenePk::attackFailToMe(rev , this);
					}
*/
				}
					...
					case Cmd::ATTACKTYPE_U2N:
						{
							...
							if(pDef)
							{
								...
								/*
								   SceneUser *pMaster = (SceneUser *)pDef->getMaster();
								   if (pMaster)
								   {
								   if (pMaster == this || !this->isEnemy(pMaster))
									{
										ScenePk::attackFailToMe(rev , this);
										return true;
									}
...
bool ScenePk::attackDeathNpc(SceneEntryPk *pAtt , SceneNpc *pDef)
{
	...
	//if (pDef->AIC) pDef->AIC->on_die(pAtt);
...
void SceneUser::sendGiftEquip(WORD level)
{
	...
	if (charbase.accPriv&ACCPRIV_GREEN_EQUIP_AT_5_25_50)
	{
		if (level==5)
		{
			...
			Channel::sendSys(this, Cmd::INFO_TYPE_GAME, "一套%u级绿色装备已经发送到您的邮箱，请到传递者处领取您的礼物~请继续加油！", level);
		}
		if (level==25)
		{
			EquipMaker maker(NULL);
		...
		if (level==50)
		{
			...
			Channel::sendSys(this, Cmd::INFO_TYPE_GAME, "一套%u级绿色装备已经发送到您的邮箱，请到邮差处领取您的礼物~请继续加油！", level, level);
		}
	}
	else//5、15级送黄金卡的装备
		...
		if (level==5)
		{
			...
			if (base)
			{
				...
				if (o)
				{
					...
					o->data.bind = 1;
					sendMail("征途活动中心", 0, name, id, Cmd::Session::MAIL_TYPE_SYS, 0, o, "亲爱的玩家:\n\t这里是我们赠送的一套黄金装备，希望能帮助您快速升级，感谢您的支持");
			...
			Channel::sendSys(this, Cmd::INFO_TYPE_GAME, "一套%u级黄金装备已经发送到您的邮箱，请到传递者处领取您的礼物~请继续加油！", level, level);
		}
		if (level==15)
		{
			EquipMaker maker(NULL);
			...
			Channel::sendSys(this, Cmd::INFO_TYPE_GAME, "一套%u级黄金装备已经发送到您的邮箱，请到邮差处领取您的礼物~请继续加油！", level, level);
		}
	}
	else//5,15级送体验卡装备
	if (charbase.accPriv&ACCPRIV_NEWBIE_EQUIP_AT_5_15)
	{
		if (level==5)
		{
			...
			Channel::sendSys(this, Cmd::INFO_TYPE_GAME, "一套VIP会员%u级专用装备已经发送到您的邮箱，请到传递者处领取您的礼物~请继续加油！", level);
		}
		if (level==15)
		{
			EquipMaker maker(NULL);
			...
			Channel::sendSys(this, Cmd::INFO_TYPE_GAME, "一套VIP会员%u级专用装备已经发送到您的邮箱，请到邮差处领取您的礼物~请继续加油！", level);
		}
	}
}
...
bool SceneUser::upgrade(DWORD num)
{
	...
	Zebra::logger->trace("用户(%s(%ld))升到%ld级" , this->name , this->id , charbase.level);
