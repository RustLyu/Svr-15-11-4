...
bool SceneUser::do_trade_rs_cmd(const Cmd::stTradeUserCmd *rev,unsigned int cmdLen)
{
	...
	if (!target || target->privatestore.step() != PrivateStore::BEGIN) {
		Zebra::logger->debug("%s(%ld)请求购买摆摊物品的用户不存在或者没有摆摊", name, id);
		return true;
	}
		...
		case REQUEST_TRADE_USERCMD_PARAMETER:
			{
				...
				if(tradeorder.canRequest())
				{
					...
					{
						Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)向自己请求交易",name,id);
						return true;
					}
					...
					Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)不能请求交易时请求交易",name,id);
				return true;
			}
			break;
		case ANSWER_TRADE_USERCMD_PARAMETER:
			{
				...
				if(tradeorder.canAnswer())
				{
					...
					if(pAsker->tradeorder.canAnswer() && pAsker->tradeorder.target()==this)
					{
						...
						Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)交易应答另一个用户",name,id);
				}
				else
					Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)不能应答交易时应答交易",name,id);
				return true;
			}
			break;
		case COMMIT_TRADE_USERCMD_PARAMETER:
			{
				if(tradeorder.commit())
				{
					...
					Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)不能确定交易时确定交易",name,id);
				return true;
			}
			break;
		...
		case ADD_OBJECT_TRADE_USERCMD_PARAMETER:
			{
				...
				if (cmd->x > TradeOrder::WIDTH || cmd->y > TradeOrder::HEIGHT) {
					Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)请求交易的物品坐标非法",name,id);	
					return true;	
				}

				zObject* ob = packs.uom.getObjectByThisID(cmd->object.qwThisID);
				if (!ob) {
					Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)请求交易的物品不存在",name,id);	
					return true;
				}

				if (ob->base->kind == ItemType_Money && cmd->object.dwNum > ob->data.dwNum  ){
					Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)请求交易的物品数量非法",name,id);	
					return true;
				}

				if (ob->data.bind || ob->data.dwObjectID == 800 || ob->base->kind == ItemType_Quest) {
					Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)用户试图交易不能交易的物品",name,id);						
					return true;
				}

				if (ob->data.pos.loc() != Cmd::OBJECTCELLTYPE_COMMON) {
					Zebra::logger->debug("[交易:玩家<------>玩家]%s(%ld)用户试图交易不在主包裹中的物品",name,id);
					return true;
				}
		...
		case VISITNPC_TRADE_USERCMD_PARAMETER:
			{
				...
				if (sceneNpc && this->canVisitNpc(sceneNpc))/*( (sceneNpc->id>=5000&&sceneNpc->id<=6000)  || (sceneNpc->scene && (sceneNpc->scene->getCountryID() == charbase.country || changeface)) ) */
				{
					...
					if(sceneNpc->scene && sceneNpc->scene == this->scene && 
							this->scene->zPosShortRange(this->getPos(),sceneNpc->getPos() , SCREEN_WIDTH , SCREEN_HEIGHT))
					{		
...
//						}
					...
					{
						Zebra::logger->trace("用户%s(%d)检查Npc(%lu)距离不合法(%u,%d,%d),(%u,%d,%d)",this->name,this->id,sceneNpc->id,this->scene->id,this->getPos().x,this->getPos().y,sceneNpc->scene->id,sceneNpc->getPos().x,sceneNpc->getPos().y);
					}
		...
		case BUYOBJECT_NPCTRADE_USERCMD_PARAMETER:
			{
				...
				if(base)
				{
					...
					{
						//this->npcTradeGold(ptCmd, base, 0);
						...
						|| ((getGoodnessState()==Cmd::GOODNESS_6) && (NPC_TYPE_MOBILETRADE==n->npc->kind)) )
				{
					...
					//	if (base->kind == ItemType_HORSE && charbase.level<30)
						...
						DWORD taxMoney = (DWORD)((price*ptCmd->dwNum*n->scene->getTax()/100.0f)+0.5f); // 买东西收税
							...
							if (NpcTrade::getInstance().verifyNpcAction(npc_dwNpcDataID, item))
							{
								if (base->kind == ItemType_HORSE) {
									...
									if (packs.removeMoney(need,"买东西")) {
										...
										Zebra::logger->trace("[交易:玩家<------商店]用户(%s,%u)买马(%d)花费银子%d",name , id ,base->id, need);
									}else {
								...
								if (base->kind == ItemType_Store) {
									...
									if (packs.removeMoney(need,"买东西") ) {
										Zebra::logger->trace("[交易:玩家<------商店]用户(%s,%u)买储物箱花费银子%d",name , id , need);
								...
								if(o)
								{
									...
									if (callback.num() || free) {
										...
										if(base->kind == ItemType_DoubleExp)
										{
											...
											Zebra::logger->trace("[交易:玩家<------商店]用户(%s,%u)买%s(%d)个花费荣誉点%d",name , id , o->name , count , price*count);
										}
										else 
										{
											...
											Zebra::logger->trace("[交易:玩家<------商店]用户(%s,%u)买%s(%d)个花费银子%d",name , id , o->name , count , price*count+taxMoney);
										}
									} 
				...
				if(srcobj)
				{
					if(this->getGoodnessState() != Cmd::GOODNESS_6)
					{
						...
						if (NpcTrade::getInstance().verifyNpcAction(npc_dwNpcDataID, item))
						{
							...
							{
								//FunctionTimes times(20,"SELLOBJECT_NPCTRADE_USERCMD_PARAMETER");
								notifyEquipChange();
								setupCharBase();
								Cmd::stMainUserDataUserCmd  userinfo;
								full_t_MainUserData(userinfo.data);
						...
						{
							Zebra::logger->debug("[交易:玩家------>商店]不能在这里卖这样物品 %u, %u, %s",
...
*/
			...
			if (privatestore.step() == PrivateStore::START) {
				//FunctionTimes times(21,"START_SELL_USERCMD_PARAMETER");
		...
		case FINISH_SELL_USERCMD_PARAMETER:
			{
				//FunctionTimes times(22,"FINISH_SELL_USERCMD_PARAMETER");
		...
		case REQUEST_ADD_OBJECT_SELL_USERCMD_PARAMETER:
			{
				...
				for(int i = 0; i < cmd->num && i < 200; i++)
				{
					...
					if (cmd->list[i].x > PrivateStore::WIDTH || cmd->list[i].y > PrivateStore::HEIGHT) {
						Zebra::logger->debug("[交易:玩家------>摆摊]%s(%ld)请求摆摊的物品坐标非法",name,id);	
						return true;	
					}

					zObject* ob = packs.uom.getObjectByThisID(cmd->list[i].qwThisID);
					if (!ob) {
						Zebra::logger->debug("[交易:玩家------>摆摊]%s(%ld)请求摆摊的物品不存在",name,id);	
						return true;
					}
					...
					if (ob->data.bind || ob->data.dwObjectID == 800 || ob->base->kind == ItemType_Quest) {
						Zebra::logger->debug("[交易:玩家------>摆摊]%s(%ld)用户试图使用不能摆摊的物品",name,id);						
						return true;
					}


					if (ob->data.pos.loc() != Cmd::OBJECTCELLTYPE_COMMON && ob->data.pos.loc()!=Cmd::OBJECTCELLTYPE_PACKAGE) {
						Zebra::logger->debug("[交易:玩家------>摆摊]%s(%ld)用户试图摆摊不在主副包裹中的物品",name,id);
						return true;
					}
	...
	/*
				if (ob->base->kind == ItemType_Money && cmd->object.dwNum > ob->data.dwNum  ){
					Zebra::logger->debug("%s(%ld)请求交易的物品数量非法",name,id);	
					return true;
				}
	*/
				if (mask.is_use(ob)) {
		...
		case REQUEST_SELL_INFO_USERCMD_PARAMETER:
			{
				...
				if(cmd->temp_id == tempid) {
					//Zebra::logger->debug("[交易:玩家<------摆摊]%s(%ld)请求自己的摆摊信息", name, id);
					return true;
				}
				
				SceneUser *target = scene->getUserByTempID(cmd->temp_id);
				if (!target || target->privatestore.step() != PrivateStore::BEGIN) {
					Zebra::logger->debug("[交易:玩家<------摆摊]%s(%ld)请求摆摊信息的用户不存在或者没有摆摊", name, id);
					return true;
				}
