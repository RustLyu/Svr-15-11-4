...
/*
		...
		if (((SceneUser *)m)->mask.is_masking())
			snprintf((char *)send.pstrChat, MAX_CHATINFO, "蒙面人 劫镖成功，获得银子%d", _gold);	
		else
			snprintf((char *)send.pstrChat, MAX_CHATINFO, "%s劫镖成功，获得银子%d", m->name, _gold);	
...
*/
		...
		snprintf(info,  MAX_CHATINFO, "你抢劫成功，获得银子"); 
